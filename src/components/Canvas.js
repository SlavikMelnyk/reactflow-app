...
const Canvas = () => {
  ...
  useEffect(() => {
    setNodes(prev => prev.map(el => {
      const block = allBlocks.find(newEl => el.blockId === newEl.blockId);
      return {
        ...el,
        ...block,
        data: {
          ...block.data,
          ports: block.data.ports.map(p => p.id ? p : { ...p, id: nextId('port-')})
        },
          style: {background: nodesBgColor || getTypeColor(el.type)}
        }
    }));
  ...
  const onDrop = useCallback( event => {
    ...
    const newNode = {
      ...
      data: {
        ...node.data,
        ports: node.data.ports?.length ? node.data.ports.map(p => ({ ...p, id: nextId('port-')})) : [],
      },
