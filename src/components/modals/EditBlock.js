...
import { allAvailableNodeTypes, idPrefixes, isCustomNodeType, portTypes } from '@/utils/consts';
...
const EditBlockModal = ({ element, onSave, onCancel, onRemove }) => {
    ...
    const handleSave = useCallback(() => {
        onSave({ ...element, type: element.type || newNodeType, data: { ...element.data, label, ports, identifier: Math.ceil(Math.random()*9) }});
    }, [onSave, element, label, ports, newNodeType]);
